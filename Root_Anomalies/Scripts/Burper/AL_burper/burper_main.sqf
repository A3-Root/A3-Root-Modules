...
if (_anti_burper_device	!="") then {
	...
	[_object_anom_burp, _anti_burper_device, _killrange] execvm "\Root_Anomalies\Root_Burper\AL_burper\burper_remove.sqf";
};
...
if (_device_detector !="") then {
	...
	if (_isaipanic) then 
	{
		[_object_anom_burp] execvm "\Root_Anomalies\Root_Burper\AL_burper\burper_ai_avoid.sqf";
	};
	if (_isaipanic) then 
	{
		[_object_anom_burp] execvm "\Root_Anomalies\Root_Burper\AL_burper\burper_ai_avoid.sqf";
	...
	if (_isaipanic) then 
	{
		[_object_anom_burp] execvm "\Root_Anomalies\Root_Burper\AL_burper\burper_ai_avoid_vizible.sqf"};
	};

if (_damage_protect !="") then {obj_prot_burper=_damage_protect; publicVariable "obj_prot_burper"};

[_object_anom_burp, _burper_radius, _vehicle_allowed, _anti_burper_device] execvm "\Root_Anomalies\Root_Burper\AL_burper\burper_damage_trap.sqf";
