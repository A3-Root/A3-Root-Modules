...
while {(alive _object_anom_burp)&&(_task_time<7)} do {
	...
	if (count _device_anti_burp >0) then {_task_time=_task_time+1; [[_object_anom_burp],"\Root_Anomalies\Root_Burper\AL_burper\burper_disable.sqf"] remoteExec ["execVM"]} else {_task_time = 0};
	sleep 5;
};
//["charge_b",true] remoteExec ["playsound"];
[[_object_anom_burp],"\Root_Anomalies\Root_Burper\AL_burper\burper_blast.sqf"] remoteExec ["execVM"];
